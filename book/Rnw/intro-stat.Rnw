...
Next, we will get the sample's mean, trimmed mean, and median. Notice that  in this example the mean is greater then trimmed mean, which is greater than the median. In general, the mean is not a great estimate of the `center' of a non-symmetric distribution. We say that the mean is more `sensitive to extreme values' than the median.
