...
Other ways to estimate some sort of `central tendency' of a distribution are:
 \begin{itemize}
 \item The trimmed mean, which is similar to the mean, except that the smallest and largest $p\cdot 100\%$  of the values are excluded from the computation. For example, if we take $p=0.1$ with the simulated data, only 800 data points are used in the calculation of the trimmed mean; 
...
Let's generate some data, and use what we've learned so far to plot a histogram of the data.
...
Next, we will get the sample's mean, trimmed mean, and median. Notice that the mean is greater then trimmed mean, which is greater than the median. In general, the mean is not a great estimate of the `center' of a non-symmetric distribution. We say that the mean is more `sensitive to extreme values' than the median.
...
$$Var(X)=\frac{(x_1-\mu)^2+\ldots+(x_n-\mu)^2}{n-1}$$
\why{Is the denominator $n$ in this case?} where $\mu$ is the mean of the distribution of $X$. Notice that in the denominator we have n-1, and not n. In words, the variance is the sum of squared deviations from the mean, divided by the sample size minus 1.
