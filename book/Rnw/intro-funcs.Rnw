...
setwd("~/Desktop")
@
The \verb|~/| notation is a shortcut to your home directory. Using this shortcut is convenient because if you are using different computers, the home directory may be different on each one.
...
There are also many functions to handle text. The \code{paste()} function attaches two strings of characters together. Note the usage of the collapse option (also called `argument') in the second example:
