...
\begin{minted}[frame=lines, framesep=2mm, baselinestretch=1.2, bgcolor=gray!20,fontsize=\footnotesize,linenos]{R}
n <- 200
cat("Number of Heads is: ", sum((runif(n) < 0.5)), "\n")
\end{minted}

Statistical inference is based on a mental exercise in which we ask, if we could repeat the same experiment infinitely many times, what would we see? With simulations, we can get a good approximation. For example, the 200 coin-tosses experiment can be repeated, say, 1,000 times. One way is to use loops, like in the following example:
\begin{minted}[frame=lines, framesep=2mm, baselinestretch=1.2, bgcolor=gray!20,fontsize=\footnotesize,linenos]{R}
n <- 200  # the number of coin-tosses in each experiment
reps <- 10000  # the number of experiments
Heads <- rep(0, reps)  # A vector to store the results (initialize with 0s)
for (i in 1:reps) {
  Heads[i] <- sum((runif(n) < 0.5))
}
hist(Heads, breaks=30)
\end{minted}
We've used a \texttt{for} loop, with an index variable called \texttt{i} which runs from 1 to 10,000. In each iteration we draw $n=200$ random numbers from a uniform distribution. For each value which turned out to be less than 0.5, \texttt{runif(n) < 0.5} is set to TRUE. Otherwise, \texttt{runif(n) < 0.5} is set to FALSE. When we use the \texttt{sum} function, the TRUE values count as 1 each, while FALSE ones counts as 0, so the result is the number of Heads. The results is stored in the $i$-th position in the vector we called \texttt{Heads}.
